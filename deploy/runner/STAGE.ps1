...
# Queue-origin automatic candidate and immutable retry workflow.
if ($Mode -in @("Execute", "Resume")) {
    $autoConfig = Get-Content -LiteralPath $ConfigPath -Raw | ConvertFrom-Json
    $autoJobPath = Join-Path (Join-Path ([string]$autoConfig.spool_root) "processing") ($JobId + ".json")
    if (Test-Path -LiteralPath $autoJobPath -PathType Leaf) {
        $autoJob = Get-Content -LiteralPath $autoJobPath -Raw | ConvertFrom-Json
        $autoWorkflow = $autoJob.PSObject.Properties["workflow"]
        if ($null -ne $autoWorkflow) {
            if ($autoWorkflow.Value -ne "candidate-recovery-v1") { throw "Unsupported automatic workflow" }
            & (Join-Path $PSScriptRoot "AUTO-RECOVER.v1.ps1") -JobId $JobId -ConfigPath $ConfigPath
            exit $LASTEXITCODE
        }
    }
}
