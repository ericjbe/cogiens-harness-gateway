  ...
  async #executeRun(job, run, input) {
    ...
    try {
      ...
      for await (const event of record.adapter.startRun({ actor: "gateway" }, session, { prompt: { text: input.prompt } })) {
        ...
        if (event.payload?.error) run.error = redactSecrets(event.payload.error);
        run.updated_at = new Date().toISOString();
        await this.#persist(job);
      }
...
function publicJob(job) {
  ...
  for (const run of snapshot.runs ?? []) {
    if (!run.error) run.error = [...(run.events ?? [])].reverse().find(event => event.payload?.error)?.payload.error ?? null;
  }
